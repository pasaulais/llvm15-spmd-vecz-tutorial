...
\begin{frame}{Memory Addressing Modes}

\begin{itemize}
    \item $Stride = 0$
    \begin{itemize}
        \item Offset is \uniform{uniform}
        \item Generate a regular \emph{scalar} load or store
    \end{itemize}
    
    \item $Stride = 1$
    \begin{itemize}
        \item Packetized offset is a sequence like $<0, 1, 2, 3>$
        \item Generate a regular \emph{vector} load or store
    \end{itemize}
    
    \item $Stride > 1$
    \begin{itemize}
        \item Packetized offset is a sequence like $<0, 4, 8, 12>$
        \item Generate an \emph{interleaved} load or store
    \end{itemize}
    
    \item Variable stride
    \begin{itemize}
        \item Packetized offset can be any sequence
        \item Generate a \emph{gather} load or \emph{scatter} store
    \end{itemize}
    
    \item Handle negative strides by reversing the vector
\end{itemize}
