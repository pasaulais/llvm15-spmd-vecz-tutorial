...
entry:
  ...
  \varying[%arrayidx] = getelementptr i32 addrspace(1)* \uniform[%src], i32 \varying[%tid]
  ...
  \varying[%arrayidx2] = getelementptr i32 addrspace(1)* \uniform[%dst], i32 \varying[%tid]
...
\begin{codebox}[commandchars=\\\[\]]
define void @__v4_add_uniform(i32 addrspace(1)* \uniform[%dst],
    i32 addrspace(1)* \uniform[%src], i32 \uniform[%alpha]) {
entry:
  \varying[%tid] = call i32 \varying[@_Z13get_global_idj(i32 0)]
  \varying[%arrayidx] = getelementptr i32 addrspace(1)* \uniform[%src], i32 \varying[%tid]
  \varying[%0] = bitcast i32 addrspace(1)* \varying[%arrayidx] to <4 x i32> addrspace(1)*
  \varying[%1] = load <4 x i32> addrspace(1)* \varying[%0], align 4

  \uniform[%sub] = sub i32 \uniform[%alpha], \uniform[1]
  \uniform[%broadcasti] = insertelement <4 x i32> undef, i32 \uniform[%sub], i32 0
  \uniform[%broadcast_sub] = shufflevector <4 x i32> \uniform[%broadcast], <4 x i32> \uniform[undef],
      <4 x i32> \uniform[zeroinitializer]

  \varying[%add] = add nsw <4 x i32> \uniform[%broadcast_sub], \varying[%1]

  \varying[%arrayidx2] = getelementptr i32 addrspace(1)* \uniform[%dst], i32 \varying[%tid]
  \varying[%2] = bitcast i32 addrspace(1)* \varying[%arrayidx2] to <4 x i32> addrspace(1)*
  store <4 x i32> \varying[%add], <4 x i32> addrspace(1)* \varying[%2], align 4
  ret void
}
\end{codebox}
